...
| `gemma4-26b-a4b-hello-comma/` | the same traces for the catalogue's `gemma-4-26b-a4b` (`unsloth/gemma-4-26B-A4B-it-qat-GGUF` `gemma-4-26B-A4B-it-qat-UD-Q4_K_XL.gguf`, SHA-256 `a7c5bc71…`, the mixture of experts, every matrix Q4_0) on the same prompt and ids: 90 layer files (three positions × 30 layers, 2,816 floats each) and `logits.f32`; the payload of `make compare-gemma4-26b-a4b-cpu` | the same harness against the same llama.cpp revision, Metal, F32 cache, 2026-09-18 (MODL-09) |
...
| `run-2026-09-12-gemma4/` | the same harness on Gemma 4 12B (`gemma-4-12b-it-UD-Q4_K_XL.gguf`, SHA-256 `90fd944d…`, `--family gemma4`): prompt arrays for 512/4,096/16,384/32,639 tokens (each starting with `<bos>` 2, then the Gemma template's `<|turn>user` prefix, the synthetic corpus, and the suffix ending in the pre-closed thought channel), responses, checksums, summary. The inputs of `make baseline-gemma4` (the K-quant entry) | `scripts/reference-baseline.py --family gemma4 --prompt-lengths 512,4096,16384,32639`, 2026-09-12, server at 32,768 context with the reference recipe's flags; summarized in [../../docs/benchmarks/reference-2026-09-12-gemma4.json](../../docs/benchmarks/reference-2026-09-12-gemma4.json) |
| `run-2026-09-12-gemma4-qat/` | the same harness on the catalogue's QAT Gemma 4 12B file (`gemma-4-12B-it-qat-UD-Q4_K_XL.gguf`, SHA-256 `90fd44e2…`, `--family gemma4`): the same four prompt lengths and construction (identical token arrays, since both files share the vocabulary and template), its own responses, checksums, and summary. The inputs of `make baseline-gemma4-qat` (`baseline-gemma4` uses the directory above) | `scripts/reference-baseline.py --family gemma4 --prompt-lengths 512,4096,16384,32639`, 2026-09-12 (MODL-08), server at 32,768 context with the reference recipe's flags; summarized in [../../docs/benchmarks/reference-2026-09-12-gemma4-qat.json](../../docs/benchmarks/reference-2026-09-12-gemma4-qat.json) |
| `run-2026-09-18-gemma4-26b-a4b/` | the same harness on the catalogue's `gemma-4-26b-a4b` (`gemma-4-26B-A4B-it-qat-UD-Q4_K_XL.gguf`, SHA-256 `a7c5bc71…`, the mixture of experts, `--family gemma4`): the same four prompt lengths and construction (token arrays byte-identical to the two 12B runs, since the three files share the vocabulary and template), its own responses, checksums, and summary. The inputs of `make baseline-gemma4-26b-a4b` | `scripts/reference-baseline.py --family gemma4 --prompt-lengths 512,4096,16384,32639`, 2026-09-18 (MODL-10), server at 32,768 context with the reference recipe's flags; summarized in [../../docs/benchmarks/reference-2026-09-18-gemma4-26b-a4b.json](../../docs/benchmarks/reference-2026-09-18-gemma4-26b-a4b.json) |
| `bonsai-hello-comma/` | the same traces for the catalogue's `bonsai-2-27b` (`prism-ml/Ternary-Bonsai-2-27B-gguf` `Ternary-Bonsai-2-27B-PQ2_0.gguf`, SHA-256 `3907dc16…`, every matrix PQ2_0 in a Hadamard-rotated basis) on `Hello,` (`[9419, 11]`, Qwen3.8's tokens): 128 layer files (two positions × 64 layers, 5,120 floats) and `logits.f32` (248,320 floats; greedy token 353, as Qwen3.8's); the payload of `make compare-bonsai-cpu` (MODL-16 session 2) | the same harness built against the **PrismML fork** `5d80cff0b8cb9f2bf823cfc4e71e3abb97f290d6` (release `prism-b10687-5d80cff`; the only decoder of the file's encodings), Metal, F32 cache, 2026-09-18 ([bonsai.md](../../docs/reference/bonsai.md#oracle-the-prismml-fork)) |
